# 往里面插入数据
from pickle import FALSE
from pymilvus import MilvusClient

client = MilvusClient(uri="http://localhost:19530")

collection_name = "create_entity_collection_name"

if client.has_collection(collection_name=collection_name):
    client.drop_collection(collection_name=collection_name)

# 创建集合(表)
client.create_collection(
    collection_name=collection_name,
    dimension = 5,
    primary_field_name="id",
    vector_field_name="vector",
    auto_id = False,
    id_type="int"
) 

# # 准备数据
# data=[
#     {"vector": [0.3580376395471989, -0.6023495712049978, 0.18414012509913835, -0.26286205330961354, 0.9029438446296592], "color": "pink_8682"},
#     {"vector": [0.19886812562848388, 0.06023560599112088, 0.6976963061752597, 0.2614474506242501, 0.838729485096104], "color": "red_7025"},
#     {"vector": [0.43742130801983836, -0.5597502546264526, 0.6457887650909682, 0.7894058910881185, 0.20785793220625592], "color": "orange_6781"},
#     {"vector": [0.3172005263489739, 0.9719044792798428, -0.36981146090600725, -0.4860894583077995, 0.95791889146345], "color": "pink_9298"},
#     {"vector": [0.4452349528804562, -0.8757026943054742, 0.8220779437047674, 0.46406290649483184, 0.30337481143159106], "color": "red_4794"},
#     {"vector": [0.985825131989184, -0.8144651566660419, 0.6299267002202009, 0.1206906911183383, -0.1446277761879955], "color": "yellow_4222"},
#     {"vector": [0.8371977790571115, -0.015764369584852833, -0.31062937026679327, -0.562666951622192, -0.8984947637863987], "color": "red_9392"},
#     {"vector": [-0.33445148015177995, -0.2567135004164067, 0.8987539745369246, 0.9402995886420709, 0.5378064918413052], "color": "grey_8510"},
#     {"vector": [0.39524717779832685, 0.4000257286739164, -0.5890507376891594, -0.8650502298996872, -0.6140360785406336], "color": "white_9381"},
#     {"vector": [0.5718280481994695, 0.24070317428066512, -0.3737913482606834, -0.06726932177492717, -0.6980531615588608], "color": "purple_4976"}
# ]

# client.insert(collection_name=collection_name, data = data)

# # 刷新&加载到内存
# client.flush(collection_name=collection_name)
# client.load_collection(collection_name=collection_name)

# result = client.query(
#     collection_name=collection_name,
#     limit = 5
#     # ids = [1,2],
#     # output_fields=["color"]
# )
# 检索出全部 此处必须设置limit 否则报错
#print("查询的结果:", result)
"""
data: ["{'id': 465132371277149528, 'vector': [0.35803765058517456, -0.602349579334259, 0.1841401308774948, -0.26286205649375916, 0.9029438495635986], 'color': 'pink_8682'}", "{'id': 465132371277149529, 'vector': [0.19886812567710876, 0.060235604643821716, 0.697696328163147, 0.2614474594593048, 0.8387295007705688], 'color': 'red_7025'}", "{'id': 465132371277149530, 'vector': [0.4374213218688965, -0.5597502589225769, 0.6457887887954712, 0.789405882358551, 0.20785793662071228], 'color': 'orange_6781'}", "{'id': 465132371277149531, 'vector': [0.31720051169395447, 0.971904456615448, -0.369811475276947, -0.48608946800231934, 0.9579188823699951], 'color': 'pink_9298'}", "{'id': 465132371277149532, 'vector': [0.4452349543571472, -0.8757026791572571, 0.8220779299736023, 0.46406289935112, 0.3033747971057892], 'color': 'red_4794'}"], extra_info: {}
"""

# upsert
upsert_data=[
    {"id": 0, "vector": [0.3580376395471989, -0.6023495712049978, 0.18414012509913835, -0.26286205330961354, 0.9029438446296592], "color": "pink_8682"},
    {"id": 1, "vector": [0.19886812562848388, 0.06023560599112088, 0.6976963061752597, 0.2614474506242501, 0.838729485096104], "color": "red_7025"},
    {"id": 2, "vector": [0.43742130801983836, -0.5597502546264526, 0.6457887650909682, 0.7894058910881185, 0.20785793220625592], "color": "orange_6781"},
    {"id": 3, "vector": [0.3172005263489739, 0.9719044792798428, -0.36981146090600725, -0.4860894583077995, 0.95791889146345], "color": "pink_9298"},
    {"id": 4, "vector": [0.4452349528804562, -0.8757026943054742, 0.8220779437047674, 0.46406290649483184, 0.30337481143159106], "color": "red_4794"},
    {"id": 5, "vector": [0.985825131989184, -0.8144651566660419, 0.6299267002202009, 0.1206906911183383, -0.1446277761879955], "color": "yellow_4222"},
    {"id": 6, "vector": [0.8371977790571115, -0.015764369584852833, -0.31062937026679327, -0.562666951622192, -0.8984947637863987], "color": "red_9392"},
    {"id": 7, "vector": [-0.33445148015177995, -0.2567135004164067, 0.8987539745369246, 0.9402995886420709, 0.5378064918413052], "color": "grey_8510"},
    {"id": 8, "vector": [0.39524717779832685, 0.4000257286739164, -0.5890507376891594, -0.8650502298996872, -0.6140360785406336], "color": "white_9381"},
    {"id": 9, "vector": [0.5718280481994695, 0.24070317428066512, -0.3737913482606834, -0.06726932177492717, -0.6980531615588608], "color": "purple_4976"}
]

client.upsert(
    collection_name=collection_name,
    data=upsert_data
)
client.flush(collection_name=collection_name)

upsert_data_query_result = client.query(
    collection_name=collection_name,
    filter="id in [0,1,2]", # 使用filter此种方式和ids效果一样
    #ids = [0,1,2],
    limit = 5,
    output_fields=["id", "color"]
)
print("upsert_data_query_result是:", upsert_data_query_result)
# 结果符合预期
"""
data: ["{'id': 0, 'color': 'pink_8682'}", "{'id': 1, 'color': 'red_7025'}", "{'id': 2, 'color': 'orange_6781'}"], extra_info: {}
"""

# 删除数据
delete_result = client.delete(
    collection_name=collection_name,
    ids = [0]
)
print("delete_result:", delete_result)

# 此处如果不加flush 直接执行下面的query 两次的结果是一样 删除的id=0的记录还是会被查出来 因为delete只是标记删除状态 异步还得一会
# 此个过程和关系数据库不一样
client.flush(collection_name=collection_name)

query_result = client.query(
    collection_name=collection_name,
    filter="id in [0,1,2]", # 使用filter此种方式和ids效果一样
    #ids = [0,1,2],
    limit = 5,
    output_fields=["id", "color"]
)
print("query_result:", query_result)


